...
void SolveCubic(double  a,
                ...
                double *x)
{
//    std::cout<< "ENTER SOLVE CUBIC " << std::endl;
    long double    a1 = b/a, a2 = c/a, a3 = d/a;
    long double    Q = (a1*a1 - 3.0*a2)/9.0; //Q = (3.0*a2 - a1*a1)/9.0;
    long double R = (2.0*a1*a1*a1 - 9.0*a1*a2 + 27.0*a3)/54.0; // R = (-(27.0*a3) + a1*(9.0*a2 + 2.0*a1*a1))/54.0;
    double    R2_Q3 = R*R - Q*Q*Q; //R2_Q3 = Q*Q*Q - R*R

    double    theta;
    double epsilon = 1e-7;
    if (fabs(Q) < epsilon) //Q = 0
    {
        //std::cout<<"(fabs(Q) < epsilon)" << std::endl;
        /* critical case where the inflexion point has zero slope */
        /* simplified solution */
        if (fabs(R2_Q3) <= epsilon){
            *solutions = 3;
            x[0] = x[1] = x[2] = - a1/3.0;
        }
        else{
    ...
    {
        //std::cout<<"(fabs(Q) > epsilon)" << std::endl;
        if (R2_Q3 <= 0.0) // 3 solutions are real and unequal
        {
            *solutions = 3;
            double Q3 = Q*Q*Q;
            if (Q3<=0)
